...
namespace SeetaFace6Sharp.Example.VideoForm
{
    public partial class UserInfoForm : Form
    {
        ...
        private void FaceDetector(Bitmap bitmap)
        {
            _ = Task.Run((Action)(() =>
            {
                SeetaFace6SharpFactory faceFactory = new SeetaFace6SharpFactory(bitmap.Width, bitmap.Height);
                try
                {
                    SetUIStatus(false);
                    ...
                    using (FaceImage faceImage = bitmap.ToFaceImage())
                    {
                        ...
                        FaceMarkPoint[] markPoints = faceFactory.Get<FaceLandmarker>().Mark(faceImage, faceInfos[0]);
                        if (markPoints == null)
                        {
                            throw new Exception("检测人脸信息失败：标记人脸失败！");
                        }
                        PlotMaskResult maskResult = faceFactory.Get<MaskDetector>().Detect(faceImage, faceInfos[0]);
