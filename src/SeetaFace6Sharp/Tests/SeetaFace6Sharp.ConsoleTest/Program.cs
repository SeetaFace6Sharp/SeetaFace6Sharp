...
namespace SeetaFace6Sharp.ConsoleTest
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //GlobalConfig.SetDefaultDeviceType(DeviceType.GPU);
            GlobalConfig.SetLog(msg =>
            {
                Console.WriteLine(msg);
            });

            GlobalConfig.ThreadNumber = 8;
        ...
        private static void FaceTrackTest()
        {
            using var bitmap = ConvertImage(imagePath);
            using FaceLandmarker faceMark = new FaceLandmarker();
            using FaceTracker faceTrack = new FaceTracker(new FaceTrackerConfig(bitmap.Width, bitmap.Height));
            Worker((sw, i) =>
            {
        ...
        private static float[] GetExtract(FaceRecognizer faceRecognizer, FaceDetector faceDetector, FaceLandmarker faceMark, FaceImage bitmap)
        {
